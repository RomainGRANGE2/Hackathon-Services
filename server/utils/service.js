    ...
    const updateService = async function(id, body, userId) {
        try {
            ...
            if (serviceToUpdate.user_id != userId) {
                throw new Error('Non autorisé: vous n\'êtes pas le propriétaire de ce service')
            }
            ...
            if (body.tag !== undefined) {
                ...
                updateValues.push(Array.isArray(body.tag) ? body.tag : [])
                paramCount++
            }
            ...
            const result = await query(
                `UPDATE "service" SET ${updateFields.join(', ')} WHERE id = $${paramCount} RETURNING *`,
                updateValues
            )
