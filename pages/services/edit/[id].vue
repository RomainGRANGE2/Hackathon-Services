...
onMounted(async () => {
  try {
    // Récupérer les détails du service à modifier
    ...
    toast.add({
      severity: 'error',
      summary: 'Erreur',
      detail: 'Une erreur s\'est produite lors de la récupération des détails du service',
      life: 3000
    })
    setTimeout(() => router.push('/services/list'), 2000)
  } finally {
...
const updateService = async () => {
  try {
    ...
    if (!serviceForm.title || !serviceForm.description || serviceForm.price < 0) {
      toast.add({
        severity: 'warn',
    ...
    if (response.success) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Erreur',
        detail: response.error || 'Une erreur s\'est produite lors de la mise à jour du service',
        life: 3000
      })
    }
  } catch (error) {
...
<template>
  <div class="container mx-auto p-4">
    ...
    <div class="flex items-center mb-6">
      <button @click="navigateBack" class="p-2 mr-3 rounded-full hover:bg-gray-100">
    ...
    <div v-else-if="!isOwner" class="text-center py-8">
      <p class="text-gray-500">Vous n'êtes pas autorisé à modifier ce service.</p>
    </div>
    <div v-else class="max-w-2xl mx-auto">
      <form @submit.prevent="updateService" class="space-y-6">
        <div class="p-field">
          <label for="title" class="block text-sm font-medium text-gray-700 mb-1">Titre *</label>
          <InputText id="title" v-model="serviceForm.title" class="w-full" placeholder="Titre du service" required />
        </div>

        <div class="p-field">
          <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description *</label>
          <Textarea id="description" v-model="serviceForm.description" class="w-full" rows="5" placeholder="Description du service" required />
        </div>

        <div class="p-field">
          <label for="price" class="block text-sm font-medium text-gray-700 mb-1">Prix (€) *</label>
          <div class="relative">
            <InputNumber id="price" v-model="serviceForm.price" class="w-full" min="0" placeholder="0" />
          </div>
        </div>
        ...
        <div class="p-field">
          ...
          <div class="flex flex-wrap gap-2 mb-2">
            <Chip v-for="(tag, index) in serviceForm.tag" :key="index" :label="tag" removable @remove="removeTag(index)" />
          </div>
          <InputText class="w-full" placeholder="Ajouter un tag (Entrée pour valider)" @keydown="addTag" />
        </div>

        <div class="flex justify-end pt-4">
          <Button type="button" class="p-button-text mr-2" @click="navigateBack">Annuler</Button>
          <Button type="submit" :loading="submitting">Enregistrer les modifications</Button>
        </div>
      </form>
    </div>
  </div>
