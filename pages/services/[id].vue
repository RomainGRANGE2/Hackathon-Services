  ...
  try {
    ...
    if (response.success) {
      ...
      if (service.value.tag && typeof service.value.tag === 'string') {
        try { service.value.tag = JSON.parse(service.value.tag) } catch (e) { service.value.tag = [] }
  ...
  try {
    ...
    if (response.success) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Erreur',
        detail: response.error || 'Une erreur s\'est produite lors de la mise à jour du statut',
        life: 3000
      })
    }
  } catch (error) {
  ...
  try {
    ...
    if (response.success) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Erreur',
        detail: response.error || 'Une erreur s\'est produite lors de la suppression du service',
        life: 3000
      })
    }
  } catch (error) {
...
<template>
  <div class="container mx-auto p-4">
    ...
    <div v-else class="bg-white rounded-lg shadow-md max-w-4xl mx-auto">
      <div class="h-64 bg-gray-200 flex items-center justify-center rounded-t-lg overflow-hidden">
      ...
      <div class="p-6">
        ...
        <div class="flex flex-wrap gap-2 mb-4" v-if="service.tag && service.tag.length > 0">
        ...
        <div v-if="isOwner" class="flex flex-wrap gap-3 mt-4">
          ...
          <Button
            ...
            :class="service.status === 'active' ? 'p-button-warning' : 'p-button-success'"
            @click="updateServiceStatus(service.status === 'active' ? 'pending' : 'active')"
            :loading="updating"
          />
          <Button
    ...
    <Dialog
      ...
      <template #footer>
        <Button label="Non" icon="pi pi-times" class="p-button-text" @click="confirmDelete = false" />
        <Button label="Oui" icon="pi pi-check" class="p-button-danger" @click="deleteService" :loading="updating" />
      </template>
    </Dialog>
  </div>
</template>
